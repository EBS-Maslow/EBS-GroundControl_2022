...
class SerialPortThread(MakesmithInitFuncs):
    ...
    def getmessage (self):
        ...
        try:
            self.serialInstance = serial.Serial(self.data.comport, 57600, timeout = .25)
        except:
            pass
        else:
            ...
            while True:
                ...
                if time.time() - self.lastMessageTime > 2:
                    ...
                    if self.data.uploadFlag:
                        self.data.message_queue.put("Message: USB connection lost. This has likely \n  caused the machine to loose it's calibration, \n  which can cause erratic behavior. It is recommended \n  to stop the program, remove the sled, and \n  perform the chain calibration process. Press \n  Continue to override and proceed with the cut.")
                    else:
                        self.data.message_queue.put("It is possible that the serial port selected \n  is not the one used by the Maslow's \n  Arduino, or that the firmware is not loaded \n  on the Arduino.")
